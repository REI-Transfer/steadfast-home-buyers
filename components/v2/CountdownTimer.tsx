...
function Block({ num, unit, size }: { num: string; unit: string; size: "sm" | "lg" }) {
  ...
  return (
    <div
      className={`rounded-lg text-center border border-gray-200 bg-gray-50 ${
        isLg ? "px-4 py-2.5 min-w-[64px]" : "px-3 py-1.5 min-w-[48px]"
      }`}
    >
      <div className={`font-black text-gray-900 leading-none tabular-nums ${isLg ? "text-2xl" : "text-lg"}`}>
        {num}
      </div>
      <div className={`uppercase tracking-widest text-gray-500 mt-1 ${isLg ? "text-[10px]" : "text-[9px]"}`}>
        {unit}
      </div>
    </div>
  )
